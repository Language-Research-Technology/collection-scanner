...
One PNG per **register** entry, encoding that entry's `@id` — these are the physical codes the app's scanner actually matches against. Regenerate with:

```bash
npm run generate:codes
```

This reads `data/register/ro-crate-metadata.json` and overwrites the PNGs in this directory.
